...
import { PLATFORM_FEE_PERCENTATE } from "@/constants";
...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure
    .mutation(async ({ ctx }) => {
      const user = await ctx.db.findByID({
        ...
        depth: 0, // user.tenant[0] is going to be a stirng (tenant ID)
      });
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const platformFeeAmount = Math.round(
      totalAmount * (PLATFORM_FEE_PERCENTATE / 100)
    );
    ...
    const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = 
      products.docs.map((product) => ({
        quantity: 1,
        price_data: {
          unit_amount: product.price * 100, // Stripe handles prices in cents 0.1 = 0.2 = 3,
          currency: "usd",
          product_data: {
            name: product.name,
            metadata: {
